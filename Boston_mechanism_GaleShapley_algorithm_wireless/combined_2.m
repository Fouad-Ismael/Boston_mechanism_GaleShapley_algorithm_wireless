...
%     rewq = [];
    ...
    for zz = 1:0.5*10^3
      ...
      % since backhaul delay is pareto distribution
        ...
        channel_gain = sqrt(2/pi)*raylrnd(0.21,L+N,L+N).*((distance1+eps).^(-path_loss_exponent));
        ...
        PSR_up_2= (1-(0.5*(sqrt(((sinr_up_2)./(sinr_up_2+1)))))).^number_packets; % equation (1) in paper walid saad
