...
fdsa = [zeros(1000,45)];
fdsa2 = [zeros(1000,45)];
cell_utilize_tot = [zeros(1000,L)];
cell_utilize_tot_PSR = [zeros(1000,L)];
...
rewq_2_PSR = [];
        ...
        for i = 1:L
            if isempty(cell_association.([string('cell'),num2str(i)]))
               ...
               rewq_2 = [rewq_2, (15*(log(1+60*(1-PSR_up_2(cell_association.([string('cell'),num2str(i)])(jj),i)))))];
                end
               ...
               cell_utilize_PSR(i) = length(cell_association_PSR.([string('cell'),num2str(i)]));
                ...
                for jj = 1:length(cell_association_PSR.([string('cell'),num2str(i)]))
                    ...
                    rewq_1_PSR= [rewq_1_PSR  94.2-Idi(i)-12-15*(log(1+60*(1-PSR_up_2(cell_association_PSR.([string('cell'),num2str(i)])(jj),i))))];
                             rewq_2_PSR = [rewq_2_PSR, 15*(log(1+60*(1-PSR_up_2(cell_association_PSR.([string('cell'),num2str(i)])(jj),i))))];
...
avg_utilization_SBS(z) =sum((mean(cell_utilize_tot(:,L_M+1:L))) )/N;
avg_utilization_SBS_PSR(z) =sum((mean(cell_utilize_tot_PSR(:,L_M+1:L))) )/N;
...
hold on
plot(20+((1:z)-1)*10,max_num_iter-1)%(-60-(((1:z)-1)*10))
figure
plot(20+((1:z)-1)*10,avg_utilization_macro_PSR,'r-')
hold on
plot(20+((1:z)-1)*10,avg_utilization_macro)%(-60-(((1:z)-1)*10))
figure
plot(20+((1:z)-1)*10,avg_utilization_SBS_PSR,'r-')
hold on
plot(20+((1:z)-1)*10,avg_utilization_SBS)%(-60-(((1:z)-1)*10))