            ...
            for i = 1:N
                if ( is_NOT_associated(1,i) == 0)
                    continue;
                        ...
                        rank = PSR_up_2(i,application);% application evaluated by cell
...
%%%%%%%%%% boston school choice %%%%%%%%%%%%%%%%%%%%
        ...
        for e = 1:L
            if (sum(is_NOT_associated_PSR)==0)
                ...
                for i_PSR = 1:N
                    if  (is_NOT_associated_PSR(i_PSR)==0)
                        continue;
                    else
                ...
                for i = 1:L
                    if isempty([cell_associate_boston.(([string('cell'),num2str(i)]))])
                        ...
                        if (length([cell_associate_boston.(([string('cell'),num2str(i)]))])<= quota_actual(i))
                        cell_association_PSR.([string('cell'),num2str(i)]) = [cell_association_PSR.([string('cell'),num2str(i)]), cell_associate_boston.(([string('cell'),num2str(i)]))];
                          ...
                          cell_association_PSR.([string('cell'),num2str(i)]) = [cell_association_PSR.([string('cell'),num2str(i)]), cell_associate_boston.(([string('cell'),num2str(i)]))(index(1:quota_actual(i)))];
                            end
                        end
                        
                          quota_actual(i) = qouta(i) - length(cell_association_PSR.([string('cell'),num2str(i)]));                                     
                        is_NOT_associated_PSR(cell_association_PSR.(([string('cell'),num2str(i)]))) = 0;
                    end
                end
                end
            end
...
%             
%             for i = 1:N
%                 if  (is_NOT_associated1(1,i)==0)
%                     continue;
%                 else
%                     application = user_preference1.([string('user'),num2str(i)])(e);
%                     rank_PSR = cell_benfit_UP(i,application);
%                     if length(cell_association1.([string('cell'),num2str(application)]))+1<= (qouta(1,application))
%                         cell_association1.([string('cell'),num2str(application)]) =[cell_association1.([string('cell'),num2str(application)]) i];
%                         cell_association1_rank.([string('cell'),num2str(application)]) =[cell_association1_rank.([string('cell'),num2str(application)]) rank_PSR];
%                         is_NOT_associated1(1,i) =0;
%                     else
%                         [p,j] = min(cell_association1_rank.([string('cell'),num2str(application)]));
%                         if rank_PSR > p
%                             x = cell_association1.([string('cell'),num2str(application)])(j);
%                             cell_association1.([string('cell'),num2str(application)])(j)=i;
%                             cell_association1_rank.([string('cell'),num2str(application)])(j)=rank_PSR;
%                             is_NOT_associated1(1,i) = 0;
