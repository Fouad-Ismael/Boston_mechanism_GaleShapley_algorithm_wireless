...
average_intrference_noise = (10^(-110/10));% in milliwats
slot_time = 20;%(in ms)
epson3 = 177.3;
...
%              user_preference_PSR.([string('user'),num2str(i)]) =PSR_up_2_index(i,:);
        ...
        for i = 1:L
            ...
            cell_association_rank.([string('cell'),num2str(i)]) =[];
             cell_association_transfer.([string('cell'),num2str(i)]) =[];
            cell_association_transfer_rank.([string('cell'),num2str(i)]) =[];
...
%             % exists)
%             for i = 1:N
%                 if (has_preference(i)==0) || ( is_NOT_associated(1,i) == 0)
%                     continue;
%                 else
%                     
%                         application = user_preference.([string('user'),num2str(i)])(1);% submit application
%                         rank = cell_benfit_UP(i,application);% application evaluated by cell
%              cell_association_transfer.([string('cell'),num2str(application)])=[cell_association_transfer.([string('cell'),num2str(application)])  i];
%                         cell_association_transfer_rank.([string('cell'),num2str(application)])=[cell_association_transfer_rank.([string('cell'),num2str(application)]) rank];
%                 end
%             end
%             for i = 1:L
%                 if isempty(cell_association_transfer.([string('cell'),num2str(i)]))
%                     continue;
%                 else
%               pool_users = [cell_association.([string('cell'),num2str(i)]) cell_association_transfer.([string('cell'),num2str(i)])];
%               pool = [cell_association_rank.([string('cell'),num2str(i)]) cell_association_transfer_rank.([string('cell'),num2str(i)])];
%                if length(pool) <= qouta(1,i)
%                    cell_association.([string('cell'),num2str(i)]) = pool_users;
%                    cell_association_rank.([string('cell'),num2str(i)]) = pool;
%                    is_NOT_associated(1,pool_users) = 0;
%                else
%                                   [sorted,index]= sort([pool],2,'descend');
%                                   cell_association.([string('cell'),num2str(i)]) = pool_users(index(1:qouta(1,i)));
%                                    cell_association_rank.([string('cell'),num2str(i)]) = pool(index(1:qouta(1,i)));
%                                   is_NOT_associated(1,cell_association.([string('cell'),num2str(i)])) = 0;
%                                    is_NOT_associated(1,pool_users(index(qouta(1,i)+1:end))) = 1;
%                                 for f = qouta(1,i)+1:length(pool)
%                                                                      user_preference.([string('user'),num2str(pool_users(index(f)))]) = user_preference.([string('user'),num2str(pool_users(index(f)))])(2:end);
%                                                                 has_preference(pool_users(index(f)))=~isempty(user_preference.([string('user'),num2str(pool_users(index(f)))]));
% 
%                                 end
%                end
%                 end
...
%             end
%             for i_PSR = 1:N
%                 if (has_preference_PSR(i_PSR)==0)|| (is_NOT_associated_PSR(1,i_PSR)==0)
%                     continue;
%                 else
...
%         utilize2 = [utilize2 count2];
        ...
        %
%        average_R_factor_f(zz) = min((minrf_t));
%         if sum(rewq_1<0)
